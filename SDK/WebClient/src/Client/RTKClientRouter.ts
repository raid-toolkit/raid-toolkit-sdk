...
class RTKClientSocket implements ISocket {
  constructor(
    ...
    private readonly socket: Promise<RaidToolkitClient>
  ) {}
  close(): void {
    this.socket.then((socket) => socket.close());
  }
  send<Channel extends string>(
    channel: Channel,
    message: any,
    context?: unknown
  ): this {
    this.socket.then((socket) =>
      socket.send([this.scope, channel, message, context])
    );
    return this;
  }
  on<Channel extends string>(
    channel: Channel,
    handler: (message: any, context?: unknown) => void
  ): this {
  ...
  off<Channel extends string>(
    channel: Channel,
    handler: (message: any, context?: unknown) => void
  ): this {
    this.socket.then((socket) =>
      socket.off("data", (handler as any).handlerWrapper)
    );
    return this;
  }
}
...
export class RTKClientRouter extends RouterBase {
  ...
  constructor(private readonly client: RaidToolkitClient) {
    ...
    this.discoverSocket = new RTKClientSocket(
      "$router/discover",
      this.clientReady
    );
  ...
  private handleDiscoverRequest = () => {
    ...
    for (const provider of this.providers) {
      definitions.push(
        ...ApiProvider.implementationsOf(provider).map((def) =>
          ApiDefinition.nameOf(def)
        )
      );
    }
