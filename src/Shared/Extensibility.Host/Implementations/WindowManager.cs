...
#pragma warning restore CS0618 // Type or member is obsolete
		...
		private object CreateFromOptions(WindowOptions? options, Type type)
		{
			if (options == null)
				return ActivatorUtilities.CreateInstance(ServiceProvider, type);
			if (options.Create != null)
				return options.Create();
			if (options.CreateInstance != null)
				return options.CreateInstance();
		...
		public IWindowAdapter<T> CreateWindow<T>() where T : class
		{
			...
			object instance = CreateFromOptions(options, typeof(T));
